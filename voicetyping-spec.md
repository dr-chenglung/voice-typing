...
- **靜音略過（2026-08-16 新增，使用者主動要求）**：停止錄音後、封裝 WAV 之前先判斷整段有沒有人說話，**整段皆為靜音就不呼叫 STT**，直接回 `Idle`。動機是使用者按下熱鍵後想不到要講什麼就直接結束，這種純靜音送進 Whisper 時**不一定回空字串**，有機會回出幻覺文字（中文語料常見的「謝謝觀看」「字幕由某某提供」），那段文字會通過既有的「辨識結果為空」檢查而被打進輸入框。
  - 判定方式（**相對比值＋持續時間，非絕對音量**）：把原始樣本切成 **30 ms** 視窗逐一算 RMS（均方根音量），取最大值當「峰值」、取第 **20 百分位數**當「底噪」。判定為「有說話」需**同時**滿足兩個條件：峰值高出底噪 **3 倍**（約 9.5 dB）以上，且超過該門檻的視窗**至少 4 個**（約 120 ms）。視窗切得短，長錄音裡只講了短短一句時峰值仍抓得到，不會被大量靜音稀釋。
  - **為何需要「持續至少 120 ms」這個條件（2026-08-16 第二次調整）**：倍數由 4.0 放寬到 3.0 是使用者要求（不希望為了讓程式認得而刻意大聲講話），但實測參考值為「不出聲 2.3 倍／正常說話 5.9 倍」，放寬後距離「不出聲」只剩約 2.3 dB——換算成絕對值，該次不出聲錄音的峰值只要從 0.00175 漲到 0.0023 就會越線，而一次呼吸、椅子聲、鍵盤聲即足以達成。故安全性改由「要持續一段時間」負責：突發雜音只佔一兩個視窗，真正的說話就算只講一個字也會持續好幾個視窗。單元測試 `an_isolated_click_is_not_treated_as_speech` 釘住此行為。
  - **副作用**：短於 120 ms 的極短促單音節（例如飛快地應一聲「嗯」）會被當成雜音丟掉。門檻訂 4 個視窗而非更多即為了盡量保留短應答（一般人刻意講一個字約 150~250 ms）；若實測發現短應答被吃掉，可調降 `MIN_SPEECH_WINDOWS`。
  - **為何是比值而不是絕對門檻（2026-08-16 改版，連調三次絕對門檻失敗後的結論）**：Windows 的麥克風音量滑桿與「麥克風增強」、不同麥克風的靈敏度、講話距離，都只是把訊號**整體乘上一個倍率**。這使得任何絕對數值都只對「某一台機器的某一組設定」成立——實測歷程即為明證：0.01 時正常說話被誤殺、0.001 時沒出聲仍生出文字、0.003 只是兩者的幾何中間值，仍隨時可能因使用者調整音量滑桿而失準。改看比值後，整體增益會**同時**放大說話與底噪，比值不變，判定因此與音量設定、麥克風型號、環境吵雜程度都無關。單元測試 `verdict_is_unchanged_when_the_whole_recording_is_amplified`（放大 0.25/1/4/10 倍結果不變）與 `steady_background_noise_is_silent_regardless_of_its_level`（底噪 0.0005～0.2 皆判定為沒說話）即在釘住這項保證。
  - **絕對安全下限** `MIN_SPEECH_PEAK_RMS = 0.0002`（約 -74 dBFS）：純比值在「幾乎全零、偶爾一個極微弱尖點」時會算出很大的比值而誤判（麥克風被系統靜音時只剩量化雜訊即為此形狀），故峰值低於此值一律判定沒說話。刻意訂得極低，以免又退化成會誤殺真實語音的絕對門檻。
  - **已知限制**：本做法量的是音量起伏，**分不出「人在講話」與「喇叭在放音樂／電視聲」**——音樂同樣有大小聲起伏，比值一樣會拉開。背景有音樂時此功能等於不作用（且 Whisper 會照樣把歌詞轉成文字）。要區分人聲與非人聲需導入真正的 VAD 模型（如 Silero／`vad-rs`），屬套件變更，需使用者另行核准，目前**未採用**。
  - **調參依據**：`stop_to_wav` 每次錄音都會印出 `[audio] 音量輪廓：峰值 RMS <值>／底噪 <值>／比值 <值> 倍／超標視窗 <n> 個 ≈ <ms> ms`，不論判定結果都印。對應函式 `audio.rs::analyze_levels`（回傳 `LevelProfile`，判定與 log 共用同一份計算，樣本只掃一趟）。注意 **release 版沒有主控台，看不到這行 log**（`main.rs` 的 `windows_subsystem = "windows"`），需用 `cargo tauri dev` 觀察。
  - **判定為沒說話時不發系統通知**，只記 log。這是第 6.3 節「所有失敗都要讓使用者看得到」的**刻意例外**：這不是失敗，使用者自己知道剛剛沒講話，跳通知只是噪音（同性質的既有例外見 6.2 的「校正無產出不通知」）。代價是麥克風被系統靜音時會表現為「按了沒反應」而無提示。
  - **「完全沒收到樣本」仍視為錯誤**（維持原本的「沒有錄到任何音訊」），因為那代表輸入裝置真的沒送資料，是該讓使用者知道的問題，與「有收到訊號但都很小聲」不同。
  - 實作：`audio.rs` 的 `is_silent()`（純函式，有單元測試）＋ `stop_to_wav()` 回傳 `Option<Vec<u8>>`；`controller.rs::process` 對應回傳 `Option<Output>`，`None` 代表本次略過。
