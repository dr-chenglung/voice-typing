...
- **靜音略過（2026-08-16 新增，使用者主動要求）**：停止錄音後、封裝 WAV 之前先判斷整段有沒有人說話，**整段皆為靜音就不呼叫 STT**，直接回 `Idle`。動機是使用者按下熱鍵後想不到要講什麼就直接結束，這種純靜音送進 Whisper 時**不一定回空字串**，有機會回出幻覺文字（中文語料常見的「謝謝觀看」「字幕由某某提供」），那段文字會通過既有的「辨識結果為空」檢查而被打進輸入框。
  - 判定方式（**相對比值，非絕對音量**）：把原始樣本切成 **30 ms** 視窗逐一算 RMS（均方根音量），取最大值當「峰值」、取第 **20 百分位數**當「底噪」，**峰值高出底噪 4 倍（約 12 dB）以上才算有說話**。視窗切得短，長錄音裡只講了短短一句時峰值仍抓得到，不會被大量靜音稀釋。
  ...
  - **調參依據**：`stop_to_wav` 每次錄音都會印出 `[audio] 音量輪廓：峰值 RMS <值>／底噪 <值>／比值 <值> 倍`，不論判定結果都印。對應函式 `audio.rs::analyze_levels`（回傳 `LevelProfile`，判定與 log 共用同一份計算，樣本只掃一趟）。注意 **release 版沒有主控台，看不到這行 log**（`main.rs` 的 `windows_subsystem = "windows"`），需用 `cargo tauri dev` 觀察。
