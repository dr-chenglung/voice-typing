...
- **靜音略過（2026-08-16 新增，使用者主動要求）**：停止錄音後、封裝 WAV 之前先判斷整段有沒有人說話，**整段皆為靜音就不呼叫 STT**，直接回 `Idle`。動機是使用者按下熱鍵後想不到要講什麼就直接結束，這種純靜音送進 Whisper 時**不一定回空字串**，有機會回出幻覺文字（中文語料常見的「謝謝觀看」「字幕由某某提供」），那段文字會通過既有的「辨識結果為空」檢查而被打進輸入框。
  - 判定方式：把原始樣本切成 **30 ms** 視窗逐一算 RMS（均方根音量），**只要任一視窗超過門檻就視為有說話**。刻意不取整段平均——長錄音裡只講了短短一句時，平均值會被大量靜音稀釋而誤判成沒說話，這種「該送卻沒送」比放過一句幻覺文字嚴重得多。
  - 門檻：RMS **0.003**（約 -50 dBFS）。這個值是 2026-08-16 兩次實機回報**夾出來的，不是理論值**：初版 0.01（-40 dBFS）時使用者正常說話被誤判為沒聲音、要「超級大聲」才過得了關，可知該裝置的說話音量**低於 0.01**；改為 0.001（-60 dBFS）後完全沒出聲的錄音仍被送去辨識並生出文字，可知其閒置底噪**高於 0.001**。兩者夾出 (0.001, 0.01)，取幾何中間值 0.003。
  - 該裝置的底噪與說話音量相當接近，**可調空間不大**；單元測試 `is_silent_returns_true_for_idle_microphone_hiss`（0.002 算沒說話）與 `is_silent_returns_false_for_speech_on_a_low_gain_microphone`（0.005 算有說話）即為此門檻的上下界，改動時務必同時確認。
  - **調參依據**：`stop_to_wav` 每次錄音都會印出 `[audio] 本次錄音最大視窗 RMS <值>（靜音門檻 <門檻>）`，不論判定結果都印。要再調整門檻時對照這個實測值，不要憑估計。對應函式 `audio.rs::max_window_rms`。
