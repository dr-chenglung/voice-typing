...
| `target_language` | 翻譯輸出的目標語言（英文語言名稱），只在翻譯模式開啟時生效 | `English` |
| `translate_mode_active` | 是否啟用翻譯模式；開啟後下一次用熱鍵開始的錄音會被翻譯，開始錄音當下讀取、不需重啟 | `false` |
...
5. 系統列圖示按右鍵可開啟：
   - **設定 Settings**：調整 API key、模型、是否校正、熱鍵、翻譯模式開關與翻譯目標語言（語音語言一律自動偵測，無需設定）。
...
在「設定」視窗勾選**啟用翻譯模式**後，下一次按熱鍵錄音就會翻譯：熱鍵仍是同一支，錄音→輸出的操作方式完全不變，差別只在停止錄音後，這次輸出會改成翻譯成「設定」視窗裡指定的**目標語言**（預設英文），而非只做輕度校正。開關存檔立即生效、**不需重啟程式**；勾選/取消勾選只影響「下一次」開始的錄音，正在錄音或處理中的這一次不受影響。底部麥克風圖示會用顏色區分目前是哪一種模式：**活力橙＝一般模式、藍紫＝翻譯模式**；系統列圖示閒置時滑鼠移過去也會顯示目前是「轉錄模式」還是「翻譯模式 → {目標語言}」。
...
| `state.rs` | AppState 狀態定義；OutputMode（一般／翻譯，開始錄音當下讀取設定檔決定） |
...
- 熱鍵變更存檔後需重啟程式才會生效（`rdev::listen` 沒有乾淨的取消機制）；**翻譯模式開關則不需重啟**，存檔立即生效，於下一次開始錄音時生效。
- 熱鍵可選項目前僅 **右 Alt／右 Ctrl** 兩種。
